...
export class DeviceDetailComponent implements OnInit, OnDestroy {
  ...
  //message: string = "{\n\tid: exempluMesaj,\n\ttemperature: 32.72\n\thumidity: 50%\n}";
  messagesColumns: string[] = ['Field', 'Value', ' '];
  ...
  ngOnInit() {
    this.route.data.subscribe((data) => {
      ...
      this.deviceService
        .getFieldsForDevice(this.device.id)
        .subscribe((fields) => (this.device.fields = fields));
      this.deviceService
        .getActionsForDevice(this.device.id)
        .subscribe((actions) => (this.device.actions = actions));
      console.log(this.device);
    });
    this.displayMessage();
    this.messageService.onSignalRActionError.subscribe((action: Action) => {
    ...
    this.messageService.onSignalRFieldError.subscribe((field: Field) => {
  ...
  deleteFieldRowData(row_obj) {
    ...
    this.deviceService.deleteFieldFromDevice(this.device.id, fieldId).subscribe(
      (result: Field) => {
        this.device.fields = this.device.fields.filter(
          (obj) => obj.id !== fieldId
        );
  ...
  addFieldRowData(row_obj) {
    var field: Field = {
      name: row_obj.name,
      type: row_obj.type,
      statistics: row_obj.statistics,
      code: row_obj.code,
    };
    this.deviceService.addFieldToDevice(this.device.id, field).subscribe(
      (result: Field) => {
        result.error = null;
  ...
  updateFieldRowData(row_obj) {
    ...
    this.deviceService.updateField(field).subscribe(
      (result: Field) => {
        console.log(result);
        this.deviceService
          .getFieldsForDevice(this.device.id)
  ...
  addActionRowData(row_obj) {
    ...
    console.log(action);
    this.deviceService.addActionToDevice(this.device.id, action).subscribe(
      (result: Action) => {
  ...
  updateActionRowData(row_obj) {
    ...
    var action: Action = {
      name: row_obj.name,
      id: row_obj.id,
      deviceId: row_obj.deviceId,
      code: row_obj.code,
    };
    this.deviceService.updateAction(action).subscribe(
      (result: Action) => {
        console.log(result);
        this.deviceService
          .getActionsForDevice(this.device.id)
          .subscribe((actions) => {
            this.device.actions = actions;
          });
  ...
  deleteActionRowData(row_obj) {
    ...
    this.deviceService
      ...
      .subscribe(
        (result: Field) => {
          this.device.actions = this.device.actions.filter(
            (obj) => obj.id !== actionId
          );
  ...
  getMessageValue(fieldName: string): string {
    ...
    var value: string = values?.find((x) => x?.dataField?.name === fieldName)?.[
      'value'
    ];
    if (value === undefined) {
      const f = this.currentValues.find((d) => d.dataField === fieldName);
      return f?.value ?? '-';
    } else {
      var fieldValue: Value = {
        dataField: fieldName,
        value: value,
        date: this.messageService.getMessage()?.date,
      };
      fieldValue.dataField = fieldName;
      fieldValue.value = value;
      this.currentValues = this.currentValues.filter(
        (obj) => obj.dataField !== fieldName
      );
  ...
  addCollaborator() {
    ...
    this.deviceService
      .addCollaboratorToDevice(this.device.id, this.collaboratorEmail)
      .subscribe(
        (m: User) => {
          console.log(m);
          this.device.collaborators.push(m);
          this.alertify.success(
            this.collaboratorEmail + ' was added successfully as a collaborator'
          );
  ...
  removeCollaborator(collab: User) {
    console.log(this.collaboratorEmail);
    this.deviceService
      .deleteCollaboratorFromDevice(this.device.id, collab.id)
      .subscribe(
        () => {
          this.device.collaborators = this.device.collaborators.filter(
            (obj) => obj.id !== collab.id
          );
          this.alertify.success(
            collab.email + ' was removed as a collaborator'
          );
        },
        (error) => {
          this.alertify.error(error);
